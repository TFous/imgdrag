...
;(function($){
    $.fn.dragZoom=function(options){
        ...
        //商品左侧大图切换
        $(".imgshow-ul li").on("click",function(){
            options.dzSon.css({
                'width':'600px',
                'height':'600px'
            });
            imgzoom();
            $(".imgshow-ul li").removeClass("on-drag");
            var imgurl=$(this).addClass("on-drag").find("img").attr("src");
            var newimg=imgurl.substring(0,imgurl.lastIndexOf("_w50"))+"_w400"+imgurl.substring(imgurl.lastIndexOf("."),imgurl.length);
            $(".dragimg").attr("src",imgurl);
        });
        //图片最大最小宽高
        function judgeSize(){
        ...
        function imgzoom(){
            options.dzSon.css({
                'left':'50%',
                'top':'50%',
                'margin-left':'-'+options.dzSon.width()/2+'px',
                'margin-top':'-'+options.dzSon.height()/2+'px'
            });
        }
